...
资料放入文件夹不等于已经读完。告诉 Agent“把新资料收进工作台”，或直接提出与它有关的问题，它会在这次任务中读取并更新入口；工作台不会在会话之外自行监控文件。
...
首次连接最小工作台后，从一个问题和手头材料开始，结构随实际使用生长。此前内部测试完成了从散放材料建台、形成比较成果、根据新反馈改判与新会话接续；当前首次引导修订的验证另见记录，历史测试不代替本次验收。尚未完成外部真实用户验收。已知模型仍可能漏读长材料，不能保证每次判断完整可靠。详见[验证范围与已知限制](docs/testing.md)。
