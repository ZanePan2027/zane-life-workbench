...
仅用于本包新建的独立工作台。职场版初始化使用 `--mode career`，组织经营版使用 `--mode work`，人生版使用 `--mode life`；三种模式首次都可加`--minimal`建立最少入口，事项格式相同。Python 3.9+，无第三方依赖、无网络。脚本相对整理器 Skill 目录。初始化要求父目录已存在、目标工作台尚不存在；需要时先建立用户授权位置的父目录，不在已有工作台上重新初始化。
...
事件可添加artifacts列表，每项包含label（成果名称）和path（工作区内实际文件相对路径）。工具检查文件存在，概览显示中性的成果编号；敏感领域从原事项按需打开。更新仍使用原ID和revision。成果正文登记实际来源，让新反馈能定位受影响材料。
